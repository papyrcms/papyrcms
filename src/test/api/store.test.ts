    ...
    describe('/[id]', () => {
      it('gets a product by its slug', async () => {
        const { data: found, status } = await axios.get(
          `${rootURL}/api/store/products/mocha-test-product`,
          axiosConfig
        )
    ...
    describe('/[id]', () => {
      it("adds a product to the user's cart", async () => {
        ...
        const { data: user } = await axios.get(
          `${rootURL}/api/auth/currentUser`,
          axiosConfig
        )

        expect(status).to.equal(200) && expect(cart).to.eql(user.cart)
      }).timeout(10000)
